# -*- coding: utf-8 -*-
"""
Created on Fri May 24 17:32:07 2024

@author: Freja Rølle Jakobsen
"""
# -*- coding: utf-8 -*-
"""
Created on Fri May 24 15:21:06 2024
...
# Load data
df_Thor = pd.read_csv('UTM_filter_coordinates_Thor.csv', delimiter=',')

# Make the points for the edge of the polynomial as UTM
x_site = np.array([
    425952.52938162, 399264.27845715, 402011.00000448, 414995.91755658,
     425648.99996396, 425944.99994539, 425939.26961863, 425701.99996057,
     425266.00002062, 425636.0000112,  425643.50043925, 425692.69058576,
     425743.57375579, 425862.958722,   426099.99996202, 425952.52938162,
])

y_site = np.array([
    6232327.54411943, 6232327.54466016, 6236670.00034547, 6252007.1231418,
     6264590.00063697, 6258540.00053956, 6258428.93084647, 6253830.00126722,
     6247230.00089146, 6240830.00127335, 6240711.35237941, 6239933.21408254,
     6239128.29526696, 6237239.74871879, 6233490.00058655, 6232327.54411943
])
...
x_init = np.random.uniform(df_Thor['utm_easting'].min(), df_Thor['utm_easting'].max(), size=num_turbines)
y_init = np.random.uniform(df_Thor['utm_northing'].min(), df_Thor['utm_northing'].max(), size=num_turbines)
...
#for a diameter of 236m and hub height of 129.8
gen_wt = GenericWindTurbine('G14MW', 236, 129.8, power_norm=14000, turbulence_intensity=.1,ws_cutin=3, ws_cutout=32)

#For a wind site from Global Wind Atlas
#punkter fundet i andet script
Thor_site = GlobalWindAtlasSite(56.325632691225955,7.652360113085295, height=130, roughness=0.1, ti=0.1)

n_wt = len(x_init) #Number of turbines
wt = gen_wt #WT model (generated Thor 14MW WT)
site = Thor_site #Define wind at site
wf_model = NOJ(site, wt) #N.O. Jensen wake model for the WF
aep = wf_model(x_init,y_init).aep() #AEP model of this WF
...
def calculate_steel_cost(water_depth, **kwargs):
...
def LCOE_CAPEX(AEP, monopile_cost, **kwargs):
  ...
  cost_CAPEX = cost_WT * n_wt * rated_power + monopile_cost # €
...
water_depth_component = CostModelComponent(input_keys=[('x', x_init), ('y', y_init)],
                                           n_wt=n_wt,
                                           cost_function=lambda x, y: water_depth_func(x, y, df=df_Thor),
                                           objective=False,
...
support_structure_component = CostModelComponent(input_keys=[('water_depth', 30*np.ones(n_wt))],
                                          n_wt=n_wt,
                                          cost_function=calculate_steel_cost,
                                          objective=False,
                                          output_keys=[('monopile_cost',0)])


LCOE_CAPEX_component = CostModelComponent(input_keys=[('AEP',0), ('monopile_cost',0)],
                                          n_wt=n_wt,
...
cost_comp = TopFarmGroup([aep_component, water_depth_component, support_structure_component, LCOE_CAPEX_component, LCOE_OPEX_component, LCOE_component])
...
problem_RS = TopFarmProblem(design_vars={'x': x_init, 'y': y_init},
                         driver=EasyRandomSearchDriver(randomize_func=RandomizeTurbinePosition_Circle(max_step=31000), max_time=10000, disp=False),
                  cost_comp=cost_comp,
                  constraints=[XYBoundaryConstraint(np.asarray([x_site, y_site]).T, boundary_type='polygon'),
...
xs = np.linspace(df_Thor['utm_easting'].min(), df_Thor['utm_easting'].max(), 100)
ys = np.linspace(df_Thor['utm_northing'].min(), df_Thor['utm_northing'].max(), 100)

XX, YY = np.meshgrid(xs, ys)

res = problem_RS.smart_start(XX, YY, aep_component.get_aep4smart_start(ws=10.6))
...
recorderRS.save('Scenario2_Thor' + time_stamp + '_RS')
...
# Save recorder to file
recorderSLSQP.save('Scenario2_Thor' + time_stamp + '_SLSQP')
...
def plot_seabed_and_turbines(df, original_turbine_locations, optimized_turbine_locations, optimized_x, optimized_y, LCOE_optimized):
    ...
    plt.title(f'Placement of turbines at Thor\n Optimization of LCoE without CO2 eq.-taxation\n LCoE after optimization: {LCOE_optimized:.2f} €/MWh'.format(LCOE_init, LCOE_optimized), pad=10)
...
plot_seabed_and_turbines(df_Thor, np.column_stack((x_init, y_init)), np.column_stack((optimized_x, optimized_y)), optimized_x, optimized_y, LCOE_optimized)

# Plot only optimized turbine locations with seabed details
plot_optimized_turbine_locations(df_Thor, optimized_x, optimized_y, LCOE_optimized)
